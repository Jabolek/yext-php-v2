...
class ResponseError implements ArrayAccess
{
    ...
    const TYPE_NON_FATAL_ERROR = 'NON_FATAL_ERROR';
    ...
    {
        return [
            self::TYPE_FATAL_ERROR,
            self::TYPE_NON_FATAL_ERROR,
            self::TYPE_WARNING,
        ];
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['type'], $allowed_values)) {
            $invalid_properties[] = "invalid value for 'type', must be one of 'FATAL_ERROR', 'NON_FATAL_ERROR', 'WARNING'.";
        }
    ...
    /**
     * Sets code
     * @param int $code Code that uniquely identifies the error or warning.  **For a list of error codes and their meanings, see the \"Error Messages\" page in the Support section of this site.**
     * @return $this
     */
    ...
    {
        $allowed_values = array('FATAL_ERROR', 'NON_FATAL_ERROR', 'WARNING');
        if (!is_null($type) && (!in_array($type, $allowed_values))) {
            throw new \InvalidArgumentException("Invalid value for 'type', must be one of 'FATAL_ERROR', 'NON_FATAL_ERROR', 'WARNING'");
        }
