 ...
 * @link        https://github.com/swagger-api/swagger-codegen
 */
class AvailableService implements ArrayAccess
{
    ...
    protected static $setters = [
        ...
        'minimum_usage' => 'setMinimumUsage',
        'minimum_usage_unit' => 'setMinimumUsageUnit',
        'add_on_to' => 'setAddOnTo'
    ];
    ...
    protected static $getters = [
        ...
        'minimum_usage' => 'getMinimumUsage',
        'minimum_usage_unit' => 'getMinimumUsageUnit',
        'add_on_to' => 'getAddOnTo'
    ];
    ...
    const MINIMUM_USAGE_UNIT_DAYS = 'DAYS';
    const MINIMUM_USAGE_UNIT_MONTHS = 'MONTHS';
     ...
     * @return string[]
     */
    public function getMinimumUsageUnitAllowableValues()
    {
        return [
            self::MINIMUM_USAGE_UNIT_DAYS,
            self::MINIMUM_USAGE_UNIT_MONTHS,
        ];
    }
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['minimum_usage'] = isset($data['minimum_usage']) ? $data['minimum_usage'] : null;
        $this->container['minimum_usage_unit'] = isset($data['minimum_usage_unit']) ? $data['minimum_usage_unit'] : null;
        $this->container['add_on_to'] = isset($data['add_on_to']) ? $data['add_on_to'] : null;
    }
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['minimum_usage']) && ($this->container['minimum_usage'] < 1)) {
            $invalid_properties[] = "invalid value for 'minimum_usage', must be bigger than or equal to 1.";
        }

        $allowed_values = ["DAYS", "MONTHS"];
        if (!in_array($this->container['minimum_usage_unit'], $allowed_values)) {
            $invalid_properties[] = "invalid value for 'minimum_usage_unit', must be one of 'DAYS', 'MONTHS'.";
        }
    ...
    public function valid()
    {
        ...
        if ($this->container['minimum_usage'] < 1) {
            return false;
        }
        $allowed_values = ["DAYS", "MONTHS"];
        if (!in_array($this->container['minimum_usage_unit'], $allowed_values)) {
    ...
    /**
     * Sets agreement_id
     * @param int $agreement_id The ID for the agreement that this service is offered under. This is important only if you have multiple active agreements with Yext, and otherwise can be ignored.
     * @return $this
     */
     ...
     * @return int
     */
    public function getMinimumUsage()
    {
        return $this->container['minimum_usage'];
    }

    /**
     * Sets minimum_usage
     * @param int $minimum_usage The minimum amount of time a service has to be active before it can be canceled. In the units specified by *minimumUsageUnit*.
     * @return $this
     */
    public function setMinimumUsage($minimum_usage)
    {

        if (!is_null($minimum_usage) && ($minimum_usage < 1)) {
            throw new \InvalidArgumentException('invalid value for $minimum_usage when calling AvailableService., must be bigger than or equal to 1.');
        }

        $this->container['minimum_usage'] = $minimum_usage;

        return $this;
    }

    /**
     * Gets minimum_usage_unit
     * @return string
     */
    public function getMinimumUsageUnit()
    {
        return $this->container['minimum_usage_unit'];
    }

    /**
     * Sets minimum_usage_unit
     * @param string $minimum_usage_unit
     * @return $this
     */
    public function setMinimumUsageUnit($minimum_usage_unit)
    {
        $allowed_values = array('DAYS', 'MONTHS');
        if (!is_null($minimum_usage_unit) && (!in_array($minimum_usage_unit, $allowed_values))) {
            throw new \InvalidArgumentException("Invalid value for 'minimum_usage_unit', must be one of 'DAYS', 'MONTHS'");
        }
        $this->container['minimum_usage_unit'] = $minimum_usage_unit;
     ...
     * @return string[]
     */
    public function getAddOnTo()
    {
        return $this->container['add_on_to'];
    }

    /**
     * Sets add_on_to
     * @param string[] $add_on_to List of SKUs that this is an add-on to. When adding this service for a location by creating an *addRequest*, you must either supply one of these SKUs along with this one, or the location must already have one of these services.
     * @return $this
     */
    public function setAddOnTo($add_on_to)
    {
        $this->container['add_on_to'] = $add_on_to;
