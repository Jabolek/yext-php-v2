...
class ContentListCost implements ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['type'], $allowed_values)) {
            $invalid_properties[] = "invalid value for 'type', must be one of 'PRICE', 'RANGE', 'NONE', 'OTHER'.";
        }
    ...
    /**
     * Sets price
     * @param string $price Price in USD, e.g., 9.50. Must be greater than or equal to 0.0 and less than or equal to 1000000.00.
     * @return $this
     */
    ...
    /**
     * Sets range_to
     * @param string $range_to Specified only if `type` is `RANGE`. In that case, this Cost represents a `price` range from `price` to `rangeTo`. Must be greater than `price` and less than or equal to 1000000.00.
     * @return $this
     */
    ...
    /**
     * Sets other
     * @param string $other Specified only if `type` is `OTHER`. User-entered text, e.g., Market Price.
     * @return $this
     */
