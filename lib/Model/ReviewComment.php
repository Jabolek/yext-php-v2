...
class ReviewComment implements ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['author_role'], $allowed_values)) {
            $invalid_properties[] = "invalid value for 'author_role', must be one of 'BUSINESS_OWNER', 'CONSUMER'.";
        }

        $allowed_values = ["PUBLIC", "PRIVATE"];
        if (!in_array($this->container['visibility'], $allowed_values)) {
            $invalid_properties[] = "invalid value for 'visibility', must be one of 'PUBLIC', 'PRIVATE'.";
        }
